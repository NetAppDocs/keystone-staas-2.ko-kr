...
+
image:console-subscription.png["구독 탭 세부 정보 보기"]
...
** *연동된 구독*: 기본 구독에 연결된 보조 구독이 있는 경우 이 열에는 기본 구독에 대해 연결된 구독 번호가 나열됩니다. 이 열은 기본적으로 숨겨져 있으며 연결된 구독을 사용할 수 있는 경우에만 표시됩니다.
** *Keystone 버전*: 구독에 사용되는 Keystone 서비스의 버전입니다. 성능 서비스 수준에 대한 요금제 규칙은 구독 버전에 따라 다를 수 있습니다. 버전 v1에 대한 자세한 내용은 https://docs.netapp.com/us-en/keystone/index.html["Keystone 구독 서비스 | 버전 1 문서"^]을 참조하십시오. 이 열은 기본적으로 숨겨져 있으며 열 선택기 image:column-selector.png["열 선택기 아이콘"] 아이콘을 사용하여 활성화할 수 있습니다.
...
** *구독 만료까지 남은 일수*: 만료일 옆에 아이콘이 표시되어 구독 만료까지 남은 기간과 자동 갱신 여부를 알려줍니다. 이 상태는 읽기 전용이며 계약 조건을 반영합니다. 계약 세부 정보를 확인하려면 link:../integrations/subscription-timeline.html["Keystone 구독의 타임라인을 확인하세요"]을 참조하십시오.
+
자동 갱신이 꺼져 있을 경우, 아이콘 색상은 만료일이 가까워졌음을 나타냅니다.

+
*** *녹색*: 만료일까지 120일 이상 남았습니다.
*** *주황색*: 만료까지 91일에서 120일이 남았습니다.
*** *빨간색*: 만료일까지 90일 미만이 남았습니다. 서비스 중단을 방지하려면 NetApp Keystone 지원팀에 갱신 요청을 제출하십시오.
*** *회색*: 구독이 만료되었습니다.
+
자동 갱신 기능이 활성화된 구독의 경우, 자동 갱신 아이콘이 표시되어 현재 구독 기간이 종료되면 자동으로 갱신됨을 나타냅니다.
...
TIP: 고급 데이터 보호 추가 서비스에 가입한 경우, 가입 번호 옆의 툴팁을 클릭하여 MetroCluster 설정에서 파트너 가입의 추적 ID를 확인할 수 있습니다. MetroCluster 구성에서 파트너 가입별 세부 소비량을 보려면 link:../integrations/adp-tab.html["MetroCluster 구독 사용량 및 상태 보기"]를 참조하세요.
