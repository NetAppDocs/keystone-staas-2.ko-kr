...
NOTE: *자산* 페이지에는 Keystone 구독과 관련된 플랫폼을 기반으로 탭이 표시됩니다. ONTAP 의 경우 *클러스터의 노드* 및 *클러스터의 볼륨* 탭을 모두 사용할 수 있습니다. StorageGRID 의 경우, *그리드의 노드* 표는 탭을 선택하지 않아도 바로 표시됩니다. 구독에 두 플랫폼이 모두 포함된 경우 세 개의 탭을 모두 사용할 수 있습니다.
...
+
Keystone 구독 전체 클러스터의 모든 볼륨에 대한 자세한 정보를 확인할 수 있습니다. 여기에는 볼륨 이름, 구독 번호, 노드 일련 번호, QoS 정책 준수 여부, 클러스터 이름, 호스트 이름 및 성능 서비스 수준이 포함됩니다. 프로비저닝된 용량, 논리적 및 물리적 사용률, 콜드 데이터 및 핫 데이터를 모니터링할 수 있습니다. 또한 다음과 같은 개요도 제공됩니다:
